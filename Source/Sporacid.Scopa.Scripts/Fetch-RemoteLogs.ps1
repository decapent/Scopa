...
$remotePaths = @("\\VMSPPHIL\C$\Logs", "\\VMSPPLAV\C$\Logs")
...
$remotePaths | ForEach-Object {
	...
	if (Test-Path($_)) {
		...
		if ($Incremental.IsPresent) {
			...
			$files = $files | Where-Object {$_.LastWriteDate -ge $lastIncrementalDate}
		}
		...
		# Zip staging folder following [HOSTNAME_LOGTYPE_TIMESTAMP]	nomenclature.
